...
def valid_brussels_coordinates(latitude: object, longitude: object) -> bool:
    return (
        finite_number(latitude)
        and finite_number(longitude)
        and 50.7 <= latitude <= 51.0
        and 4.1 <= longitude <= 4.7
    )
...
def validate_history_payload(payload: dict, feature: str) -> list[dict]:
    ...
    for record in records:
        require(finite_number(record.get("count")) and record["count"] >= 0, f"counter history for {feature} contains an invalid count")
    ...
    require(
        all(
            all(type(record.get("time_gap")) is int and 1 <= record["time_gap"] <= 96 for record in day_records)
            and {record["time_gap"] for record in day_records} == expected_slots
            for day_records in by_day.values()
        ),
        f"counter history for {feature} must contain each integer 15-minute slot exactly once per day",
    )
    return records
...
def fetch_trees(data_dir: Path = DATA) -> None:
    ...
    out = data_dir / "brussels-trees-sample.json"
    ...
    require(
        all(valid_brussels_coordinates(row.get("latitude"), row.get("longitude")) for row in rows),
        "managed-tree sample contains coordinates outside the Brussels region",
    )
    completeness = {
    ...
    remarkable_out = data_dir / "brussels-remarkable-trees-sample.json"
    remarkable_rows = []
    ...
    require(
        all(valid_brussels_coordinates(row.get("latitude"), row.get("longitude")) for row in remarkable_rows),
        "remarkable-tree sample contains coordinates outside the Brussels region",
    )
...
def bike_device_row(feature: dict) -> dict:
    properties = feature.get("properties", {})
    coordinates = feature.get("geometry", {}).get("coordinates", [])
    require(isinstance(coordinates, list) and len(coordinates) >= 2, "bicycle-counter API returned invalid geometry")
    require(valid_brussels_coordinates(coordinates[1], coordinates[0]), "bicycle-counter API returned coordinates outside the Brussels region")
    return {
...
def fetch_bike_devices(data_dir: Path = DATA) -> None:
    ...
    rows = [bike_device_row(feature) for feature in payload.get("features", [])]
    ...
    out = data_dir / "brussels-bike-counters.json"
    write_json(out, {
...
def fetch_bike_history(data_dir: Path = DATA) -> None:
    for feature in COUNTER_HISTORY_FEATURES:
        ...
        out = data_dir / f"brussels-bike-history-{feature}-2024-01.json"
...
def build_tree_mobility_join(data_dir: Path = DATA) -> None:
    trees_path = data_dir / "brussels-trees-sample.json"
    bikes_path = data_dir / "brussels-bike-counters.json"
    ...
    out = data_dir / "brussels-tree-bike-nearest.json"
...
REFRESH_OUTPUTS = (
    ...
    *(f"brussels-bike-history-{feature}-2024-01.json" for feature in COUNTER_HISTORY_FEATURES),
    "brussels-tree-bike-nearest.json",
    "brussels-tree-counter-flow.json",
)


def promote_files(stage: Path, data_dir: Path, filenames=REFRESH_OUTPUTS, replace_file=os.replace) -> None:
    with tempfile.TemporaryDirectory(prefix=".refresh-backup-", dir=data_dir.parent) as temporary:
        ...
        try:
            for filename in filenames:
                replace_file(stage / filename, data_dir / filename)
...
def staged_refresh(data_dir: Path = DATA) -> None:
    data_dir.mkdir(parents=True, exist_ok=True)
    with tempfile.TemporaryDirectory(prefix=".refresh-", dir=data_dir.parent) as temporary:
        stage = Path(temporary) / "data"
        shutil.copytree(data_dir, stage)
        fetch_trees(stage)
        fetch_bike_devices(stage)
        fetch_bike_history(stage)
        build_tree_mobility_join(stage)
        subprocess.run([
            sys.executable,
            str(ROOT / "trees-surfaces" / "join_counter_flow.py"),
            "--data-dir",
            str(stage),
        ], check=True)
        missing = [filename for filename in REFRESH_OUTPUTS if not (stage / filename).is_file()]
        require(not missing, f"staged refresh did not produce required outputs: {missing}")
        promote_files(stage, data_dir)
